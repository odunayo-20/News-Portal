...
if ($("#summernoteExample").length) {
    $('#summernoteExample').summernote({
        ...
        callbacks: {
            onPaste: function (e) {
                // Prevent default paste for text breaking
                e.preventDefault();
                ...
                if (clipboard && clipboard.items) {
                    for (let i = 0; i < clipboard.items.length; i++) {
                        if (clipboard.items[i].type.indexOf("image") !== -1) {
                            let file = clipboard.items[i].getAsFile();
                            let reader = new FileReader();

                            reader.onload = function (event) {
                                $('#summernoteExample').summernote('insertImage', event.target.result, function ($image) {
                                    $image.css({
                                        'max-width': '100%',
                                        'height': 'auto'
                                    });
                ...
                // For text — insert line breaks automatically
                let text = clipboard.getData('text');
                text = text.replace(/(.{80})/g, "$1<br>");
                $('#summernoteExample').summernote('pasteHTML', text);
            }
        }
    });
}
